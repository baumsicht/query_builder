...
class QueryBuilderDialog(QDialog):
    ...
    def on_layer_change(self, index):
        """Wenn Layer gewechselt wird, UI komplett zurücksetzen."""
        layer_id = self.layer_combo.currentData()
        self.layer = QgsProject.instance().mapLayer(layer_id)
        self.reset_ui()

    def reset_ui(self):
        """Löscht alle Gruppen und setzt alles neu auf."""
        while self.groups:
            grp = self.groups.pop()
    ...
    def create_input_widget(self, is_date, field_name=None):
        """Return QDateEdit if date, else QLineEdit with autocomplete."""
        if is_date:
            dt = QDateEdit()
    ...
    def is_date_field(self, field_name):
        """Erkennt Datumsfelder via typeName."""
    ...
    def friendly_alias(self, f: QgsField):
        """Alias aus Layer-Metadaten oder lesbarer Feldname."""
    ...
    def add_group(self):
        ...
        # Kopf: UND/ODER, Duplizieren, Löschen
        hl = QHBoxLayout()
    ...
    def add_condition(self, group):
        ...
        btn_del_line.setToolTip("Bedingung löschen")

        for w in (fld, op, inp1, inp2, btn_del_line):
            hl.addWidget(w)
        ...
        blk.update({
            "fld": fld, "op": op, "in1": inp1, "in2": inp2,
            "del_btn": btn_del_line, "container": container, "layout": hl
        })
        ...
        btn_del_line.clicked.connect(remove_line)
    ...
    def copy_expression(self):
        """Kopiert den erzeugten Ausdruck in die Zwischenablage."""
        expr = self.preview.toPlainText()
        if expr:
            QGuiApplication.clipboard().setText(expr)

    def save_filter(self):
        """Speichert die aktuelle Gruppen-Struktur als JSON."""
        path, _ = QFileDialog.getSaveFileName(self, "Filter speichern", "", "JSON Files (*.json*)")
        if not path:
            return
        data = {"version": "v1.8", "groups": []}
    ...
    def load_filter(self):
        """Lädt eine JSON-Filter-Datei und stellt die UI wieder her."""
        ...
        # UI zurücksetzen: alle Gruppen löschen bis eine bleibt
