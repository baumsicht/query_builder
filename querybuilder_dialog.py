...
class QueryBuilderDialog(QDialog):
    def __init__(self, layer):
        super().__init__()
        self.setWindowTitle("QueryBuilder  –  v0.2.2")
        ...
        # interne Gruppen-Liste
        self.groups = []
        # erste Gruppe anlegen
        self.add_group()

        # globaler +Gruppe hinzufügen-Button
        ...
        # initiale Warnung aktualisieren
        self.update_warning()
    ...
    def create_input_widget(self, is_date, field_name=None):
        ...
        le = QLineEdit()
        if field_name:
            vals = {str(f[field_name]) for f in self.layer.getFeatures() if f[field_name] not in (None, '')}
    ...
    def add_condition(self, group):
        blk = {}; hl = QHBoxLayout()
        fld = QComboBox()
        for f in self.layer.fields(): fld.addItem(f"{self.friendly_alias(f)} ({f.name()})", f.name())
        op = QComboBox(); op.addItems(["=","!=",">","<",">=","<=","zwischen","enthält","ist leer","ist nicht leer"])
        inp1 = self.create_input_widget(False); inp2 = self.create_input_widget(False); inp2.hide()
        btn_del = QPushButton("❌"); btn_del.setToolTip("Lösche diese Bedingung")
        for w in (fld, op, inp1, inp2, btn_del): hl.addWidget(w)
        ...
        def update():
            name = fld.currentData(); is_date = self.is_date_field(name)
            operator = op.currentText()
            for w in (blk["in1"], blk["in2"]): hl.removeWidget(w); w.deleteLater()
            blk["in1"] = self.create_input_widget(is_date, name)
            blk["in2"] = self.create_input_widget(is_date, name)
            blk["in2"].setVisible(operator=="zwischen")
            hl.insertWidget(2, blk["in1"]); hl.insertWidget(3, blk["in2"])
        fld.currentIndexChanged.connect(update); op.currentTextChanged.connect(update)
        update()

        def remove_line(): group["conds"].removeWidget(container); container.deleteLater(); group["blocks"].remove(blk)
        btn_del.clicked.connect(remove_line)

    def duplicate_group(self, group):
        self.add_group(); new=self.groups[-1]; old=group["blocks"]
        w=new["conds"].takeAt(0).widget(); w.deleteLater(); new["blocks"].clear()
        for ob in old:
            self.add_condition(new); nb=new["blocks"][-1]
            nb["fld"].setCurrentIndex(ob["fld"].currentIndex()); nb["op"].setCurrentIndex(ob["op"].currentIndex())
            ...
            nb["in1"].setText(v1); nb["in2"].setText(v2)
        self.update_warning()

    def remove_group(self, group):
        if len(self.groups)<=1: return
        idx=self.groups.index(group); group["frame"].deleteLater(); self.groups.pop(idx)
        if idx==0: self.groups[0]["op"].setDisabled(True)
        self.update_warning()
    ...
    def generate_expression(self):
        ...
        for i,grp in enumerate(self.groups):
            ...
            for blk in grp["blocks"]:
                ...
                if op=="ist leer": conds.append(f"(\"{f}\" IS NULL OR \"{f}\" = '{{}}')")
                elif op=="ist nicht leer": conds.append(f"(\"{f}\" IS NOT NULL AND \"{f}\" != '{{}}')")
                elif op=="enthält": conds.append(f"\"{f}\" ILIKE '%{v1}%'" )
                elif op=="zwischen": conds.append(f"\"{f}\" >= '{v1}' AND \"{f}\" <= '{v2}'" )
                elif v1:
                    if v1.replace('.','',1).isdigit(): conds.append(f"\"{f}\" {op} {v1}")
                    else: conds.append(f"\"{f}\" {op} '{v1}'")
            expr="("+" AND ".join(conds)+")"
            if i==0: parts.append(expr)
            else:
    ...
    def load_filter(self):
        ...
        for grp in list(self.groups)[1:]: self.remove_group(grp)
        first=self.groups[0]
        for blk in list(first["blocks"]):
            widget=blk["container"]; first["conds"].removeWidget(widget); widget.deleteLater(); first["blocks"].remove(blk)
        for i,grp_data in enumerate(data.get("groups",[])):
            if i>0: self.add_group()
            grp=self.groups[i]; grp["op"].setCurrentText(grp_data.get("op","UND"))
            for blk_data in grp_data.get("blocks",[]):
                self.add_condition(grp); blk=self.groups[i]["blocks"][-1]
                ...
                if hasattr(blk["in1"],"setDate"):
                    try: d=QDate.fromString(blk_data.get("value1",""),"yyyy-MM-dd"); blk["in1"].setDate(d)
                    except: blk["in1"].setText(blk_data.get("value1",""))
                    try: d2=QDate.fromString(blk_data.get("value2",""),"yyyy-MM-dd"); blk["in2"].setDate(d2)
                    except: blk["in2"].setText(blk_data.get("value2",""))
                else: blk["in1"].setText(blk_data.get("value1","")); blk["in2"].setText(blk_data.get("value2",""))
        self.generate_expression(); self.update_warning()

    def update_warning(self):
        if len(self.groups)>1 and all(grp["op"].currentText()=="UND" for grp in self.groups[1:]): self.warn_label.show()
        else: self.warn_label.hide()
